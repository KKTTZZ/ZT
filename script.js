...
window.addEventListener('load', function() {
    ...
    const menuIcons = document.querySelectorAll('.menu-icon');    menuIcons.forEach(icon => {
        icon.addEventListener('click', function() {            const targetScreen = this.getAttribute('data-target');
            showScreen(targetScreen);        });
    });});
function showScreen(screenId) {
    // Hide all screens    const screens = document.querySelectorAll('body > div');
    screens.forEach(screen => {        screen.style.display = 'none';
    });
    // Show the target screen    document.getElementById(screenId).style.display = 'flex';
}
